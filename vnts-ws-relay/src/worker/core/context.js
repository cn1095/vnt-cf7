...
export class VntContext {
  ...
  async leave(cache) {
    ...
    if (this.link_context) {
      ...
      if (networkInfo) {
        // logger.debug(`[连接清理-网络] 找到网络信息: ${this.link_context.group}`);
        const clients = networkInfo.clients;

        // 获取客户端信息
        const clientInfo = clients.get(this.link_context.virtual_ip);
        if (clientInfo) {
          // logger.debug(`[连接清理-客户端] 找到客户端信息: ${this.link_context.virtual_ip}`);
          // 验证地址和时间戳匹配
          if (
            clientInfo.address !== this.link_address ||
            clientInfo.timestamp !== this.link_context.timestamp
          ) {
            // logger.debug(`[连接清理-验证] 地址或时间戳不匹配，跳过清理`);
            return;
          }
